    ...
    for h in [6144, 8192, 10240, 12288, 16384, 20480, 25600]:
      ...
      for syncPolicy in ['Row-Sync', 'Tile-Sync']:
        ...
        if s == -1:
          ...
          ctime = o
          cublasTimes[m] = ctime
        (s, o) = subprocess.getstatusoutput(f"./mlp {m} {int(4*h/8)} {h} {h} check=false split_k1_slices={tiles[m][h]['split_ks'][0]} split_k2_slices={tiles[m][h]['split_ks'][1]} rowSyncOrTileSync={1 if syncPolicy == 'Row-Sync' else 0}")
        # print(o)
        if "Invalid" in o:
          pass
        elif s != 0:
          ...
          print(f"{m} & {h} & {syncPolicy} & {ctime} & {avg(bTimeTotal)} & {stdev(bTimeTotal)} & {avg(bTimeMatmul1)} & {avg(bTimeMatmul2)} & {maxtbs} & {matmul1TBs} & {matmul2TBs} & {avg(otime)} & {stdev(otime)} & {100 - avg(otime)/avg(bTimeTotal)*100}")
          # btime = re.findall(r'START-BASELINE: ([\.\d]+)', o)
          # baselineTimes[m] = btime[0]
          # otime = re.findall(r'START-OVERLAPPED elapsedtime ([\.\d]+)', o)
          # overlappedTimes[m] = otime[0]
